...
class TestHarvesterActivityParser(unittest.TestCase):
    ...
    def testBasicParsing(self):
        for logs in [self.nominal_logs]:
            ...
            expected_eligible_plot_counts = [8, 3, 3, 5, 3]
            expected_proofs_found_counts = [0, 1, 1, 2, 1]
            expected_qualities_found_counts = [0, 0, 0, 0, 1]
            expected_search_times = [0.55515, 1.05515, 0.23412, 0.12348, 0.34952]
            expected_total_plots_counts = [42, 42, 42, 43, 43]
            for msg, eligible, proofs, qualities, search, total in zip(
                activity_messages,
                expected_eligible_plot_counts,
                expected_proofs_found_counts,
                expected_qualities_found_counts,
                ...
                self.assertEqual(msg.found_proofs_count, proofs, "Found proofs count don't match")
                self.assertEqual(msg.found_qualities_count, qualities, "Found qualities count don't match")
